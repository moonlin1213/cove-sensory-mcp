"""Ordinary audio coordination with optional transcript instruction."""
...
from cove_sensory_mcp.reports.schemas import AudioSensoryReport, Coverage
...
class AudioCoordinator:
    ...
    async def sense(self, input: Any) -> AudioSensoryReport:
        ...
        async with self._workspace_factory() as workspace:
            ...
            question = input.question + (
                " Include a timestamped transcript."
                if input.include_transcript
                else " Do not include a transcript."
            )
        ...
        observation = executed.observations.get(Modality.AUDIO)
        if observation is None:
            raise SensoryError(
                ErrorCode.PROVIDER_UNAVAILABLE,
                "The audio Provider returned no usable observation.",
                retryable=True,
            )
        usage = provider_usage(executed)
        return AudioSensoryReport(
            status="completed",
            coverage=Coverage(
